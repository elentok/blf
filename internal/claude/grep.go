...
// Returns a wrapped ErrRgNotFound if rg is not in PATH.
...
// GrepResultProjectLabel returns a display label for the project containing
// the given file path. It uses the directory name decoded via dedashDir.
func GrepResultProjectLabel(filePath string) string {
	return buildLabel(dedashDir(filepath.Base(filepath.Dir(filePath))))
}
