...
func TestGrepTranscriptsSingleDir(t *testing.T) {
	if _, err := os.LookupEnv("SKIP_RG_TESTS"); err == false {
		if _, e := lookupRg(); e != nil {
...
func TestGrepTranscriptsRgAbsent(t *testing.T) {
	// This test verifies the ErrRgNotFound path. We can't easily mock exec.LookPath,
	// so we skip if rg is present and just test the error type/message.
	if _, e := lookupRg(); e == nil {
		t.Skip("rg is present, cannot test absent path without mocking")
	}
	dir := grepFixtureDir(t)
	_, err := GrepTranscripts("fibonacci", []string{dir})
	if err == nil {
		t.Fatal("expected error when rg is absent")
	}
	if !strings.Contains(err.Error(), "ripgrep") {
		t.Errorf("error should mention ripgrep, got: %v", err)
	}
	if !strings.Contains(err.Error(), "brew install") {
		t.Errorf("error should mention brew install on macOS, got: %v", err)
	}
}
