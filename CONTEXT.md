...
**blf beads**:
An interactive TUI (`blf beads`) that is a direct interactive view of `bd list` for the current project's **Beads** issues, and hands a chosen issue's id to an AI agent for implementation. Defaults to `bd list`'s own filter (closed issues excluded); `blf beads -a`/`--all` mirrors `bd list --all` (closed included). Renders bd's own **issue tree** — since `bd list --json` is *not* itself tree-ordered (a child can precede its own parent in the array), blf reconstructs it client-side: group issues by their `parent` field, sort top-level and each sibling group by id ascending, reproducing exactly what plain `bd list` prints in the terminal. **Epic** rows render bold, in a distinct color, so they read as epics at a glance. Embeds the shared **fuzzy finder** widget with a side-by-side **issue preview**, treating the external `bd` CLI (its JSON output) as the sole source of truth — blf never reads the beads database directly. Its primary verb copies the selected id to the clipboard (and prints it to stdout) and quits; secondary verbs create/edit/re-state/close issues and open the dependency graph, all by shelling out to `bd`. Project-contextual: relies on `bd`'s auto-discovery of the `.beads` database from the working directory.
_Avoid_: "beads manager" (it is a browser/picker first; management is secondary), reimplementing beads' data layer (it is a thin front-end over `bd`). _Superseded_: an earlier design scoped/sorted rows by **readiness** (unblocked/blocked, via `bd ready`) with a `ctrl+f` scope cycle (actionable/ready/blocked/all) — dropped in favor of this plain `bd list` view, since readiness isn't derivable from `bd list --json` alone and the extra `bd ready` fetch/sort/cycle added a triage-queue model the user didn't want.

**issue tree** (blf beads):
The parent→child grouping blf beads reconstructs client-side from the flat `bd list --json` array (see **blf beads**). When a fuzzy query matches a descendant but not its ancestor, the ancestor row still renders — dimmed, unselectable — so the match keeps its tree context instead of appearing to float with no parent.
_Avoid_: confusing with the **issue preview**'s **subtasks tree** — that's a single issue's children fetched via `bd children` for the preview pane; the **issue tree** is the whole list's row layout.
...
- **blf beads** also embeds the **fuzzy finder** widget: it loads the working set once via `bd list --json` (`--all` when the `-a`/`--all` flag is set), builds the **issue tree** client-side, and fuzzy-matches client-side; re-fetches only on a mutation or manual refresh (no scope cycle to trigger a reload). The selected row's **issue preview** is fetched separately (lazy, debounced, cached). Pressing Enter copies the issue id and quits (a picker handoff), unlike the launcher's Enter which performs an action and stays resident.
